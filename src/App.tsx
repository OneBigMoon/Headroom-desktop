...
const collaborationBridgeCopy: Record<Exclude<Locale, "system">, { title: string; description: string; rule: string; detail: string; activation: string; notice: string; pending: string }> = {
  en: { title: "Collaboration bridges", description: "Connect external planning and review services; does not replace the primary workflow or run autonomous loops.", rule: "Can run with other groups", detail: "Lets ChatGPT plan and review while Codex retains code execution through a read-only MCP Bridge bounded to the workspace.", activation: "Click Connect ChatGPT once; later sessions reuse the connection. Switch accounts from the connected state.", notice: "Headroom prepares the local Bridge automatically. Account authorization stays in ChatGPT.", pending: "Ready to connect" },
  "zh-CN": { title: "协作桥接", description: "连接外部规划与审查服务；不替代主工作流，也不参与自动执行循环。", rule: "可与其他分组同时启用", detail: "让 ChatGPT 负责规划与审查，Codex 保留代码执行权；通过受工作区边界限制的只读 MCP Bridge 读取代码。", activation: "首次只需点击一次“连接 ChatGPT”，之后会自动复用；切换账号时点击“切换 ChatGPT 账号”。", notice: "Headroom 会自动准备本地 Bridge；账号授权只在 ChatGPT 中完成。", pending: "可以开始连接" },
...
interface CodexBridgeStatus {
  ...
  tunnelFallbackReason?: string | null;
...
const bridgeAccountCopy: Record<Exclude<Locale, "system">, {
  switch: string;
  confirm: string;
  hint: string;
}> = {
  en: {
    switch: "Switch ChatGPT account",
    confirm: "Switching ChatGPT accounts revokes this workspace's local bridge authorization. You will need to authorize again in the other ChatGPT account. Continue?",
    hint: "Headroom does not read your ChatGPT email. Click Switch first to revoke local access; then change accounts in ChatGPT and connect again here.",
  },
  "zh-CN": {
    switch: "切换 ChatGPT 账号",
    confirm: "切换 ChatGPT 账号会撤销当前工作区的本地授权，之后需要在另一个 ChatGPT 账号中重新授权。继续吗？",
    hint: "Headroom 不读取 ChatGPT 邮箱。点击“切换”会先撤销本地授权；然后在 ChatGPT 切换账号，再回到这里点击“连接”。",
  },
  "zh-TW": {
    switch: "切換 ChatGPT 帳號",
    confirm: "切換 ChatGPT 帳號會撤銷目前工作區的本機授權，之後需要在另一個 ChatGPT 帳號重新授權。要繼續嗎？",
    hint: "Headroom 不會讀取 ChatGPT 電子郵件。點擊「切換」會先撤銷本機授權；再到 ChatGPT 切換帳號，回來點擊「連線」。",
  },
  ja: {
    switch: "ChatGPT アカウントを切り替える",
    confirm: "ChatGPT アカウントを切り替えると、このワークスペースのローカル認証を取り消します。別の ChatGPT アカウントで再認証が必要です。続行しますか？",
    hint: "Headroom は ChatGPT のメールアドレスを読み取りません。「切り替え」でローカル認証を取り消してから、ChatGPT でアカウントを切り替え、ここで再接続してください。",
  },
  ko: {
    switch: "ChatGPT 계정 전환",
    confirm: "ChatGPT 계정을 전환하면 이 작업공간의 로컬 브리지 인증이 취소됩니다. 다른 ChatGPT 계정에서 다시 인증해야 합니다. 계속할까요?",
    hint: "Headroom은 ChatGPT 이메일을 읽지 않습니다. 먼저 전환을 눌러 로컬 인증을 취소한 뒤 ChatGPT에서 계정을 바꾸고 여기서 다시 연결하세요.",
  },
};

const bridgeIntegrationCopy: Record<Exclude<Locale, "system">, {
  title: string;
  description: string;
  enabled: string;
  disabled: string;
  ...
  en: {
    ...
    description: "Turn this on to let Headroom install and run the required helper automatically. Turn it off to stop automatic calls.",
    enabled: "On: Headroom may install, start, and call the local helper.",
    disabled: "Off: Headroom will not install, start, or call the local helper.",
    prerequisites: "You only need to sign in or approve access in ChatGPT when asked.",
  },
  "zh-CN": {
    title: "ChatGPT 连接",
    description: "打开后，Headroom 会自动安装并运行所需组件；关闭后不再自动安装、启动或调用。",
    enabled: "已开启：Headroom 可以自动准备并调用本地组件。",
    disabled: "已关闭：Headroom 不会自动安装、启动或调用本地组件。",
    prerequisites: "只有 ChatGPT 要求登录或授权时，才需要你操作。",
  },
  "zh-TW": {
    title: "ChatGPT 連線",
    description: "開啟後，Headroom 會自動安裝並執行必要元件；關閉後不再自動安裝、啟動或呼叫。",
    enabled: "已開啟：Headroom 可以自動準備並呼叫本機元件。",
    disabled: "已關閉：Headroom 不會自動安裝、啟動或呼叫本機元件。",
    prerequisites: "只有 ChatGPT 要求登入或授權時，才需要你操作。",
  },
  ja: {
    title: "ChatGPT 接続",
    description: "オンにすると必要なヘルパーを自動でインストールして実行します。オフにすると自動呼び出しを停止します。",
    enabled: "オン：必要なローカルコンポーネントを自動準備して呼び出します。",
    disabled: "オフ：ローカルコンポーネントを自動インストール、起動、呼び出ししません。",
    prerequisites: "ChatGPT でログインまたは許可を求められたときだけ操作が必要です。",
  },
  ko: {
    title: "ChatGPT 연결",
    description: "켜면 필요한 도우미를 자동으로 설치하고 실행합니다. 끄면 자동 호출을 중지합니다.",
    enabled: "켜짐: 필요한 로컬 구성요소를 자동으로 준비하고 호출합니다.",
    disabled: "꺼짐: 로컬 구성요소를 자동 설치, 시작 또는 호출하지 않습니다.",
    prerequisites: "ChatGPT에서 로그인 또는 권한 승인을 요청할 때만 조작하면 됩니다.",
  },
};

const bridgeTunnelCopy: Record<Exclude<Locale, "system">, {
  title: string;
  stable: string;
  temporary: string;
  configure: string;
  ...
  settingsPrerequisites: string;
  enabled: string;
  disabled: string;
}> = {
  en: {
    title: "Bridge address",
    stable: "Stable domain configured",
    temporary: "Temporary address (may change after restart)",
    configure: "Use a stable domain",
    prompt: "Enter a domain already managed in Cloudflare (for example example.com):",
    hint: "Configure once; Bridge restarts and ChatGPT account switches keep the same Connector address.",
    temporaryHint: "Configure a stable domain before connecting if you want the Connector address to survive restarts.",
    settingsTitle: "Stable Bridge address",
    settingsDescription: "Optional advanced feature. Keep it off to use a temporary address.",
    settingsPrerequisites: "Requires a Cloudflare account, a domain already managed in Cloudflare, and one-time browser authorization.",
    enabled: "Enabled",
    disabled: "Off",
  },
  "zh-CN": {
    title: "Bridge 地址",
    stable: "固定域名已配置",
    temporary: "当前是临时地址（重启后可能变化）",
    configure: "配置固定域名",
    prompt: "请输入已经接入 Cloudflare 的域名（例如 example.com）：",
    hint: "只需配置一次；Bridge 重启或切换 ChatGPT 账号都会复用同一个 Connector 地址。",
    temporaryHint: "如果希望重启后仍使用同一个 Connector 地址，请先配置固定域名。",
    settingsTitle: "Bridge 固定地址",
    settingsDescription: "可选高级功能。关闭时使用临时地址。",
    settingsPrerequisites: "前提：Cloudflare 账号、已接入 Cloudflare 的域名，以及一次性浏览器授权。",
    enabled: "已开启",
    disabled: "已关闭",
  },
  "zh-TW": {
    title: "Bridge 位址",
    stable: "固定網域已設定",
    temporary: "目前是臨時位址（重啟後可能變更）",
    configure: "設定固定網域",
    prompt: "請輸入已加入 Cloudflare 的網域（例如 example.com）：",
    hint: "只需設定一次；Bridge 重啟或切換 ChatGPT 帳號都會沿用同一個連接器位址。",
    temporaryHint: "若要讓連接器位址在重啟後保持不變，請先設定固定網域。",
    settingsTitle: "Bridge 固定位址",
    settingsDescription: "可選進階功能。關閉時使用臨時位址。",
    settingsPrerequisites: "前提：Cloudflare 帳號、已加入 Cloudflare 的網域，以及一次性瀏覽器授權。",
    enabled: "已開啟",
    disabled: "已關閉",
  },
  ja: {
    title: "Bridge アドレス",
    stable: "固定ドメインを設定済み",
    temporary: "一時アドレス（再起動後に変わる可能性があります）",
    configure: "固定ドメインを設定",
    prompt: "Cloudflare で管理しているドメインを入力してください（例: example.com）:",
    hint: "一度設定すれば、再起動や ChatGPT アカウント切り替え後も同じコネクターアドレスを使います。",
    temporaryHint: "再起動後も同じコネクターアドレスを使うには、先に固定ドメインを設定してください。",
    settingsTitle: "Bridge 固定アドレス",
    settingsDescription: "任意の詳細機能です。オフの場合は一時アドレスを使います。",
    settingsPrerequisites: "前提: Cloudflare アカウント、Cloudflare で管理するドメイン、初回のブラウザー認証。",
    enabled: "オン",
    disabled: "オフ",
  },
  ko: {
    title: "Bridge 주소",
    stable: "고정 도메인 설정됨",
    temporary: "임시 주소(재시작 후 바뀔 수 있음)",
    configure: "고정 도메인 설정",
    prompt: "Cloudflare에서 관리하는 도메인을 입력하세요(예: example.com):",
    hint: "한 번만 설정하면 재시작이나 ChatGPT 계정 전환 후에도 같은 커넥터 주소를 사용합니다.",
    temporaryHint: "재시작 후에도 같은 커넥터 주소를 사용하려면 먼저 고정 도메인을 설정하세요.",
    settingsTitle: "Bridge 고정 주소",
    settingsDescription: "선택 가능한 고급 기능입니다. 끄면 임시 주소를 사용합니다.",
    settingsPrerequisites: "필요 조건: Cloudflare 계정, Cloudflare에서 관리하는 도메인, 최초 브라우저 인증.",
    enabled: "켜짐",
    disabled: "꺼짐",
  },
};
...
export function bridgeNeedsRestart(
  status: Pick<CodexBridgeStatus, "paired" | "installed" | "running"> | null | undefined,
): boolean {
  return Boolean(status?.paired && status.installed && !status.running);
}
...
const bridgeStepCopy: Record<Exclude<Locale, "system">, string> = {
  en: "Click Connect ChatGPT below. Service preparation is automatic; confirm workspace access in the browser.",
  "zh-CN": "点击下方“连接 ChatGPT”，服务会自动准备好；在浏览器确认工作区访问权限即可。",
  "zh-TW": "按下方「連線 ChatGPT」，服務會自動準備；在瀏覽器確認工作區存取權限即可。",
  ja: "下の「ChatGPT に接続」を押すと自動で準備します。ブラウザーでワークスペースへのアクセスを確認してください。",
  ko: "아래에서 ChatGPT 연결을 누르면 자동으로 준비됩니다. 브라우저에서 작업공간 접근 권한을 확인하세요.",
};
...
const CODEX_BRIDGE_ENABLED_STORAGE_KEY = "headroom:codexBridgeEnabled";
const STABLE_TUNNEL_ENABLED_STORAGE_KEY = "headroom:stableTunnelEnabled";

function readCodexBridgeEnabled(): boolean {
  try {
    return window.localStorage.getItem(CODEX_BRIDGE_ENABLED_STORAGE_KEY) === "true";
  } catch {
    return false;
  }
}

function writeCodexBridgeEnabled(enabled: boolean): void {
  try {
    if (enabled) {
      window.localStorage.setItem(CODEX_BRIDGE_ENABLED_STORAGE_KEY, "true");
    } else {
      window.localStorage.removeItem(CODEX_BRIDGE_ENABLED_STORAGE_KEY);
    }
  } catch {
    // best effort
  }
...
function readStableTunnelEnabled(): boolean {
  try {
    return window.localStorage.getItem(STABLE_TUNNEL_ENABLED_STORAGE_KEY) === "true";
  } catch {
    return false;
  }
}

function writeStableTunnelEnabled(enabled: boolean): void {
  try {
    if (enabled) {
      window.localStorage.setItem(STABLE_TUNNEL_ENABLED_STORAGE_KEY, "true");
    } else {
      window.localStorage.removeItem(STABLE_TUNNEL_ENABLED_STORAGE_KEY);
    }
  } catch {
    // best effort
  }
...
export default function App() {
  ...
  const [codexBridgeBrowserPending, setCodexBridgeBrowserPending] = useState(false);
  ...
  const [codexBridgeEnabled, setCodexBridgeEnabled] = useState(readCodexBridgeEnabled);
  const [stableTunnelEnabled, setStableTunnelEnabled] = useState(readStableTunnelEnabled);
  ...
  useEffect(() => {
    if (!codexBridgeEnabled || codexBridgeBusy) return;
  ...
  }, [codexBridgeEnabled, effectiveCodexBridgeWorkspace, codexBridgeBusy]);
  ...
  }, [codexBridgeEnabled, codexBridgeStatus?.paired, effectiveCodexBridgeWorkspace, codexBridgeBusy]);

  useEffect(() => {
    if (
      !codexBridgeEnabled ||
      codexBridgeBusy ||
      codexBridgeStatus?.paired ||
      codexBridgeBrowserPending ||
      bridgePairingIsUsable(codexBridgeStatus, codexBridgeNow) ||
      !effectiveCodexBridgeWorkspace
    ) return;
    void handleConnectCodexBridge();
  }, [
    codexBridgeEnabled,
    codexBridgeBusy,
    codexBridgeStatus?.paired,
    codexBridgeStatus?.pairingCode,
    codexBridgeStatus?.pairingExpiresAt,
    codexBridgeBrowserPending,
    codexBridgeNow,
    effectiveCodexBridgeWorkspace,
  ]);
  ...
  useEffect(() => {
    setCodexBridgeBrowserPending(false);
  }, [effectiveCodexBridgeWorkspace, codexBridgeStatus?.paired]);

  useEffect(() => {
    if (!codexBridgeBrowserPending) return;
    const timeout = window.setTimeout(() => setCodexBridgeBrowserPending(false), 15 * 60_000);
  ...
  async function handleSwitchCodexBridgeAccount() {
    if (!window.confirm(bridgeAccountCopy[resolvedLocale].confirm)) return;
    connectedBridgeUrlRef.current = null;
    setCodexBridgeRepairNeeded(false);
    setCodexBridgeBrowserPending(false);
    const revoked = await handleCodexBridgeAction("unpair");
    if (!revoked) return;
    await openExternalLink("https://chatgpt.com/");
  }

  async function handleConfigureCodexBridgeTunnel() {
    const actionWorkspace = effectiveCodexBridgeWorkspace;
    if (!actionWorkspace) {
      setCodexBridgeError(bridgeWorkspaceUnavailableCopy[resolvedLocale]);
      return;
    }
    const zone = window.prompt(bridgeTunnelCopy[resolvedLocale].prompt, "");
    ...
    setCodexBridgeBusy("tunnel");
    setCodexBridgeError(null);
    try {
      const status = await invoke<CodexBridgeStatus>("configure_codex_bridge_tunnel", {
        mode: "named",
        zone: zone.trim(),
      ...
      setStableTunnelEnabled(true);
      writeStableTunnelEnabled(true);
      setCodexBridgeStatus(status);
    } catch (error) {
      if (
      ...
      setCodexBridgeError(describeInvokeError(error, "固定域名配置失败。"));
    } finally {
  ...
  async function handleCodexBridgeAction(action: "install" | "update" | "setup" | "doctor" | "pair" | "unpair" | "stop" | "uninstall" | "ack_endpoint"): Promise<boolean> {
    if (action === "uninstall" && !window.confirm(`确定要${bridgeUiCopy[resolvedLocale].uninstall} Codex with ChatGPT 吗？`)) return false;
    const actionWorkspace = effectiveCodexBridgeWorkspace;
    const actionWorkspaceGeneration = codexBridgeWorkspaceGenerationRef.current.generation;
    const actionRequestGeneration = ++codexBridgeStatusRequestGenerationRef.current;
    codexBridgeStatusInFlightWorkspaceRef.current = undefined;
  ...
  async function handleConnectCodexBridge() {
    ...
    try {
      ...
      if (status?.paired) {
        // A valid connector survives a stop/restart. Bring only this workspace's
        // Bridge back up; never make the user authorize the same connector again.
        if (bridgeNeedsRestart(status)) {
          status = await invoke<CodexBridgeStatus>("run_codex_bridge_action", {
            action: "setup",
            workspace: status.workspacePath?.trim() || requestedWorkspace,
          });
          if (
            codexBridgeWorkspaceGenerationRef.current.generation !== actionWorkspaceGeneration ||
            codexBridgeStatusRequestGenerationRef.current !== actionRequestGeneration
      ...
      // Choose the transport once as part of the normal connection flow.
      // Quick tunnels are the zero-setup default; a named tunnel is opt-in
      // from Settings and asks for a domain only when that switch is enabled.
      if (!status.tunnelMode || status.tunnelMode === "unset") {
        let tunnelMode: "quick" | "named" = "quick";
        let zone: string | null = null;
        if (stableTunnelEnabled) {
          zone = window.prompt(bridgeTunnelCopy[resolvedLocale].prompt, "");
          if (zone?.trim()) tunnelMode = "named";
        }
        status = await invoke<CodexBridgeStatus>("configure_codex_bridge_tunnel", {
          mode: tunnelMode,
          zone: zone?.trim() || null,
          workspace: actionWorkspace,
        });
      ...
      // Every new attempt gets a fresh browser-verification link. A previous link may be consumed.
      {
        status = await invoke<CodexBridgeStatus>("run_codex_bridge_action", {
          action: "setup",
          workspace: actionWorkspace,
      ...
      if (!displayedStatus.browserAuthSupported || !displayedStatus.authorizationUrl) {
        throw new Error(bridgeBrowserCopy[resolvedLocale].unavailable);
      }
      const authorizationUrl = new URL(displayedStatus.authorizationUrl);
      authorizationUrl.searchParams.set("lang", resolvedLocale);
      await openExternalLink(authorizationUrl.toString());
      setCodexBridgeBrowserPending(true);
    } catch (error) {
      if (
        codexBridgeWorkspaceGenerationRef.current.generation !== actionWorkspaceGeneration ||
        codexBridgeStatusRequestGenerationRef.current !== actionRequestGeneration
  ...
  async function handleOpenCodexBridgeAuthorization() {
    const existingUrl = codexBridgeStatus?.authorizationUrl;
    if (!existingUrl) {
      await handleConnectCodexBridge();
      return;
    }
    try {
      const authorizationUrl = new URL(existingUrl);
      authorizationUrl.searchParams.set("lang", resolvedLocale);
      await openExternalLink(authorizationUrl.toString());
    } catch {
      await handleConnectCodexBridge();
      ...
      >
        ...
        <div className="post-install__actions">
          ...
          <button
            ...
            onClick={() => {
              void invoke("show_dashboard_window").catch((error) => {
                console.error("show_dashboard_window failed", error);
              });
            }}
            type="button"
          >
      ...
      <section className="tray-panel">
        <header className="tray-panel__header">
          ...
          <RuntimeStatusIndicator
            className="tray-panel__local-status"
        ...
        <div className="tray-content" hidden={activeView !== "bridge"}>
          ...
          <section className="addon-group addon-group--collaboration">
            ...
            <p className="addon-card__activation-note">{bridgeStepCopy[resolvedLocale]}</p>
              ...
              <div><strong>{codexBridgeStatus?.paired ? bridgeUiCopy[resolvedLocale].authorized : (codexBridgeBrowserPending || codexBridgeStatus?.pairingCode) ? bridgeUiCopy[resolvedLocale].waiting : bridgeUiCopy[resolvedLocale].unauthorized}</strong><span>{bridgeUiCopy[resolvedLocale].authorizationHint}</span>{codexBridgeStatus?.codexLoggedIn ? <small>已检测到本机 Codex 登录态；它只用于本地 Codex，不替代 ChatGPT 工作区授权。</small> : null}</div>
            </div>
            ...
            {codexBridgeStatus && stableTunnelEnabled ? <div className="bridge-auth-card__status">
              <div className="bridge-auth-card__status-icon" aria-hidden="true">{codexBridgeStatus.tunnelStable ? "✓" : "~"}</div>
              <div>
                <strong>{bridgeTunnelCopy[resolvedLocale].title}: {codexBridgeStatus.tunnelStable ? bridgeTunnelCopy[resolvedLocale].stable : bridgeTunnelCopy[resolvedLocale].temporary}</strong>
                <span>{codexBridgeStatus.tunnelHostname || (codexBridgeStatus.tunnelStable ? bridgeTunnelCopy[resolvedLocale].hint : bridgeTunnelCopy[resolvedLocale].temporaryHint)}</span>
                {codexBridgeEnabled && stableTunnelEnabled && codexBridgeStatus.installed && !codexBridgeStatus.tunnelStable ? <button className="secondary-button" type="button" disabled={Boolean(codexBridgeBusy) || !effectiveCodexBridgeWorkspace} onClick={() => void handleConfigureCodexBridgeTunnel()}>{bridgeTunnelCopy[resolvedLocale].configure}</button> : null}
              </div>
            </div> : null}
            <details className="bridge-auth-card__advanced bridge-auth-card__advanced--setup">
              <summary>{bridgeUiCopy[resolvedLocale].advanced}</summary>
            ...
            <section className="bridge-auth-card__section">
              <div className="bridge-auth-card__section-head"><div><h3>{bridgeUiCopy[resolvedLocale].pairingStep}</h3><p>{bridgeBrowserCopy[resolvedLocale].hint}</p></div><span className="bridge-auth-card__step">1</span></div>
              <div className="bridge-auth-card__pairing-state">{codexBridgeBusy === "connect" ? bridgeBrowserCopy[resolvedLocale].preparing : codexBridgeStatus?.paired ? bridgeUiCopy[resolvedLocale].authorized : (codexBridgeBrowserPending || codexBridgeStatus?.pairingCode) ? bridgeBrowserCopy[resolvedLocale].ready : bridgeUiCopy[resolvedLocale].pairingReady}</div>
              {codexBridgeStatus?.paired ? <p className="addon-card__activation-note">重启、更新或重新打开 Headroom 会复用现有连接；{bridgeAccountCopy[resolvedLocale].hint}</p> : null}
              {codexBridgeStatus?.pairingCode && !codexBridgeStatus.paired ? <details className="bridge-auth-card__advanced"><summary>{bridgeBrowserCopy[resolvedLocale].manual}</summary><div className="bridge-auth-card__pairing-code"><code>{codexBridgeStatus.pairingCode}</code><button className="secondary-button" type="button" onClick={() => void copyCodexBridgePairingCode()}><Copy size={16} aria-hidden="true" />{codexBridgeCopied ? bridgeUiCopy[resolvedLocale].copied : bridgeUiCopy[resolvedLocale].copyCode}</button>{bridgeExpiryValue(codexBridgeStatus) !== null ? <small>{bridgeUiCopy[resolvedLocale].expires} {formatBridgeExpiry(codexBridgeStatus, resolvedLocale, codexBridgeNow)}</small> : null}</div></details> : null}
              <div className="addons-card__updates" aria-live="polite"><button className="primary-button" type="button" disabled={!codexBridgeEnabled || Boolean(codexBridgeBusy) || !effectiveCodexBridgeWorkspace || codexBridgeStatus?.paired} onClick={() => void ((codexBridgeBrowserPending || bridgePairingIsUsable(codexBridgeStatus, codexBridgeNow)) ? handleOpenCodexBridgeAuthorization() : handleConnectCodexBridge())}>{codexBridgeBusy === "connect" ? bridgeBrowserCopy[resolvedLocale].preparing : (codexBridgeBrowserPending || bridgePairingIsUsable(codexBridgeStatus, codexBridgeNow)) ? bridgeUiCopy[resolvedLocale].openChatGPT : bridgeUiCopy[resolvedLocale].connect}</button>{codexBridgeStatus?.paired ? <button className="secondary-button" type="button" disabled={!codexBridgeEnabled || Boolean(codexBridgeBusy)} onClick={() => void handleSwitchCodexBridgeAccount()}>{bridgeAccountCopy[resolvedLocale].switch}</button> : null}</div>
            </section>
            <details className="bridge-auth-card__section bridge-auth-card__service"><summary><span><strong>{bridgeUiCopy[resolvedLocale].service}</strong><small>{bridgeUiCopy[resolvedLocale].serviceHint}</small></span><span className="bridge-auth-card__step">2</span></summary>
              <div className="addons-card__updates" aria-live="polite">{!codexBridgeStatus?.installed ? <button className="secondary-button" type="button" disabled={!codexBridgeEnabled || Boolean(codexBridgeBusy)} onClick={() => void handleCodexBridgeAction("install")}>{bridgeUiCopy[resolvedLocale].install}</button> : <><button className="secondary-button" type="button" disabled={!codexBridgeEnabled || Boolean(codexBridgeBusy) || !effectiveCodexBridgeWorkspace} onClick={() => void handleCodexBridgeAction("setup")}>{bridgeUiCopy[resolvedLocale].setup}</button><button className="secondary-button" type="button" disabled={!codexBridgeEnabled || Boolean(codexBridgeBusy)} onClick={() => void handleCodexBridgeAction("update")}>{bridgeUiCopy[resolvedLocale].update}</button><button className="secondary-button" type="button" disabled={!codexBridgeEnabled || Boolean(codexBridgeBusy) || !effectiveCodexBridgeWorkspace} onClick={() => void handleCodexBridgeAction("doctor")}>{bridgeUiCopy[resolvedLocale].doctor}</button><button className="secondary-button" type="button" disabled={!codexBridgeEnabled || Boolean(codexBridgeBusy) || !codexBridgeStatus.running} onClick={() => void handleCodexBridgeAction("stop")}>{bridgeUiCopy[resolvedLocale].stop}</button><button className="secondary-button" type="button" disabled={!codexBridgeEnabled || Boolean(codexBridgeBusy)} onClick={() => void handleCodexBridgeAction("uninstall")}>{bridgeUiCopy[resolvedLocale].uninstall}</button></>}{codexBridgeBusy ? <span>{bridgeUiCopy[resolvedLocale].working}：{codexBridgeBusy}…</span> : null}</div>
        ...
        <div className="tray-content tray-content--settings" hidden={activeView !== "settings"}>
            <section className="panel-stack">
              ...
              <article className="soft-card panel-card">
                <div className="panel-card__header">
                  <div>
                    <h3>{bridgeIntegrationCopy[resolvedLocale].title}</h3>
                    <p>{bridgeIntegrationCopy[resolvedLocale].description}</p>
                  </div>
                  <button
                    aria-checked={codexBridgeEnabled}
                    aria-label={bridgeIntegrationCopy[resolvedLocale].title}
                    className={`connector-switch${codexBridgeEnabled ? " is-on" : ""}`}
                    onClick={() => {
                      const next = !codexBridgeEnabled;
                      setCodexBridgeEnabled(next);
                      writeCodexBridgeEnabled(next);
                      if (next) {
                ...
                <p className="settings-account-notice">
                  {codexBridgeEnabled ? bridgeIntegrationCopy[resolvedLocale].enabled : bridgeIntegrationCopy[resolvedLocale].disabled}
                  {" · "}
                  {bridgeIntegrationCopy[resolvedLocale].prerequisites}
                </p>
                {codexBridgeEnabled ? <details className="settings-advanced">
                  <summary>{bridgeTunnelCopy[resolvedLocale].settingsTitle}</summary>
                  <p className="settings-account-notice">{bridgeTunnelCopy[resolvedLocale].settingsDescription} {bridgeTunnelCopy[resolvedLocale].settingsPrerequisites}</p>
                  <button
                    aria-checked={stableTunnelEnabled}
                    aria-label={bridgeTunnelCopy[resolvedLocale].settingsTitle}
                    className={`connector-switch${stableTunnelEnabled ? " is-on" : ""}`}
                    onClick={() => {
                      const next = !stableTunnelEnabled;
                      setStableTunnelEnabled(next);
                      writeStableTunnelEnabled(next);
                    }}
                    role="switch"
                    type="button"
                  >
